...
class AutotunerBaseClass {
    ...
    async bayesianOptimization(objective: string = 'error', acquisitionFunction: string = 'expectedImprovement', useCrossValidation: boolean = false, maxIteration: number = 0.75, stoppingCriteria?: ((observedValues: ObservedValues, expectedImprovement: number) => boolean)) {
        ...
        this.optimizer = new bayesianOptimizer.Optimizer(this.paramspace.domainIndices, acquisitionFunction);
    ...
    async gridSearchOptimizytion(objective: string = 'error', useCrossValidation: boolean = false) {
        ...
        this.initializePriors();
        this.optimizer = new gridSearchOptimizer.Optimizer(this.paramspace.domainIndices);
